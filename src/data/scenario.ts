...
export const scenarioData: ScenarioData = {
  ...
  scoring: {
    dimensions: [
      { key: "goal_definition", name: "目标定义", weight: 1.0, description: "是否把本关真正要解决的问题说清楚" },
      { key: "business_context", name: "业务理解", weight: 1.0, description: "是否补充了足够的业务事实、数据和背景" },
      { key: "constraint_clarity", name: "约束表达", weight: 1.0, description: "是否说明预算、人手、时间、风险边界等限制" },
      { key: "ai_iteration", name: "追问迭代", weight: 1.0, description: "是否会让 AI 补洞、反问、比较方案和修正判断" },
      { key: "execution_validation", name: "落地验证", weight: 1.0, description: "是否形成可执行动作和验证指标" },
    ],
...
⚠️ 评分核心原则：本游戏评估的是学员把 AI 变成决策外援，最终做出经营决策的能力，而不是经营知识考试，也不是数据复述能力。评分必须同时检查三件事：
1. 方案匹配：最终决策/选择执行的方案，是否真正解决了本关最初提出的问题，不能答非所问。
2. AI 协作：学员是否会向 AI 交代背景、补充约束、追问风险、比较方案、要求 AI 修正，而不是只让 AI 直接给答案。
3. 商业判断：方案是否考虑客户、成本、现金流、执行资源、风险收益和验证指标，是否像一个真实小店老板能执行的经营动作。

【严格扣分规则】
- 如果最终方案没有回应本关任务要求，或者只是在泛泛讲经营建议，哪怕文字完整，总分不应超过25分。
- 如果学员几乎没有和 AI 进行有效协作，只提交单次笼统问题或直接采用 AI 输出，"追问迭代"不得超过4分。
- 如果方案缺少成本、资源、时间、风险或验证指标，"业务理解"和"落地验证"不得超过6分。
...
1. 目标定义：是否抓住本关最初问题，并让最终方案直接服务这个问题？
2. 业务理解：是否主动补充或引用了业务事实、客户、成本、合同、库存、竞争、现金流等上下文，而不是只问泛泛建议？
3. 约束表达：是否明确预算、人手、时间、风险底线、不可做事项等限制？
4. 追问迭代：是否会让 AI 反问、补充缺口、比较方案、预演风险、修正判断？
5. 落地验证：选择执行的方案是否能变成具体动作，并有验证指标或复盘方式？
...
请严格输出JSON格式，不要包含任何额外说明：
{
  ...
  "total": 总分,
  "comment": "必须用中文，按“优秀：...”和“待改进：...”两段书写。必须覆盖：是否解决本关问题、AI 协作方式、商业判断质量；如发现照搬数据或方案跑题，必须指出。"
}`,
  },
